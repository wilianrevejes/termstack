# Installs WezTerm + font on Windows and seeds machine.lua.
...
Write-Host "  bash scripts/bootstrap-linux.sh"
...
if ($repoDir -ne (Join-Path $HOME ".config\wezterm")) {
    Write-Host "The repository is not at ~\.config\wezterm. Clone it there, or set:"
    Write-Host "  setx WEZTERM_CONFIG_FILE `"$repoDir\wezterm.lua`""
}
